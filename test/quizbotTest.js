...
describe('launching the quiz (Menu handlers)', function (done){
  ...
  it('launches and then asks user to start', function (done) {
    ...
    alexa.launched(function (error, payload) {
      assert.include(payload.response.outputSpeech.ssml, 'Welcome to <emphasis level="moderate">quiz</emphasis>bot. Say start when you\'re ready.');
      done();
    });
  });
  ...
  it('can reply with help message when the user asks for help', function (done) {
    ...
    alexa.launched(function (error, payload) {
      alexa.spoken('help', function (error, payload) {
        assert.include(payload.response.outputSpeech.ssml, 'Say start to begin a new quiz or exit to close <emphasis level="moderate">quiz</emphasis>bot.');
        done();
      });
    });
  });
  ...
  it('can reply with the unhandled message when it doesn\'t understand the user\'s request', function (done) {
    alexa.launched(function (error, payload) {
      alexa.spoken('Test', function (error, payload) {
        assert.include(payload.response.outputSpeech.ssml, 'Sorry, I didn\'t catch that, say start to begin a new quiz or exit to close <emphasis level="moderate">quiz</emphasis>bot.');
        done();
      });
    });
  });
